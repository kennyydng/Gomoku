...
struct Pos {
	...
	// Exigé par la recherche : elle compare le coup mémorisé dans la table de
	// transposition à ceux de la liste courante pour l'essayer en premier.
	constexpr bool operator==( this Pos lhs, Pos rhs )
...
// Longueur d'un alignement et pas le long d'un axe : utilisés par la
// détection de menaces scalaire (voir Gomoku::threatAt).
inline int runLenOf(Pos start, Pos end, Dir dir)
	{ return dir.x ? (end.x - start.x) / dir.x + 1 : (end.y - start.y) / dir.y + 1; }
inline Pos stepPos(Pos p, Dir dir, int n)
	{ return p + dir * n; }
